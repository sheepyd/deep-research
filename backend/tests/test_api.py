...
class FakeService:
    ...
    async def list_task_summaries(self, session, limit=20):
        return [
            {
                "id": str(uuid4()),
                "parent_task_id": None,
    ...
    async def create_follow_up_task(self, parent_task_id, follow_up_request, max_results=None):
        return type("Task", (), {"id": str(uuid4()), "status": "queued"})()

    async def run_task_inline(self, payload):
    ...
    async def run_follow_up_inline(self, parent_task_id, follow_up_request, max_results=None):
        return self.FakeDetail(
            {
                "id": str(uuid4()),
    ...
    async def get_task_detail(self, session, task_id):
        return {
            "id": task_id,
            "parent_task_id": None,
            "research_iteration": 1,
